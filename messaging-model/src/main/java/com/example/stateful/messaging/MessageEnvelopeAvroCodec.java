...
public final class MessageEnvelopeAvroCodec {
    ...
    private static S fromAvro(com.example.stateful.messaging.avro.S s) {
        return new S(s.getId(), s.getPid(), s.getBDate(), s.getQ(), s.getQCarry(), s.getQA(), s.getQAOppositeDelta(), s.getQAOppositeTotal(), s.getRollover(), s.getO(), Dir.valueOf(s.getDir().name()), SCycle.valueOf(s.getSCycle().name()), s.getLedgerTime(), nullable(s.getRefS()), s.getQUnalloc(), s.getQUnkn());
    }
    ...
    private static com.example.stateful.messaging.avro.TS toAvro(TS ts) {
        return com.example.stateful.messaging.avro.TS.newBuilder()
                ...
                .setTCycle(com.example.stateful.messaging.avro.TCycle.valueOf(ts.tCycle().name()))
                .setSMode(com.example.stateful.messaging.avro.SMode.valueOf(ts.sMode().name()))
                .setAStatus(com.example.stateful.messaging.avro.AStatus.valueOf(ts.a_status().name()))
                .setO(ts.o()).setCancel(ts.cancel())
                .setQF(ts.q_f()).setLedgerTime(ts.ledgerTime())
    ...
    private static TS fromAvro(com.example.stateful.messaging.avro.TS ts) {
        return new TS(ts.getId(), ts.getPid(), nullable(ts.getPidAlt1()), nullable(ts.getPidAlt2()), ts.getTid(), ts.getSid(), nullable(ts.getAccId()), nullable(ts.getSorId()), nullable(ts.getOarId()),
                ...
                TCycle.valueOf(ts.getTCycle().name()), SMode.valueOf(ts.getSMode().name()), AStatus.valueOf(ts.getAStatus().name()), ts.getQF(), ts.getLedgerTime());
    }
