...
public final class KafkaStreamsManager implements SmartLifecycle {
    ...
    public void start() {
        ...
        kafkaStreams.start();
        running = true;
        log.info("Started processor applicationId={} bootstrapServers={} inputTopic={} outputTopic={} instancePartition={}/{}",
                settings.applicationId(), settings.bootstrapServers(), settings.inputTopic(), settings.outputTopic(),
                settings.partitionNumber(), settings.totalPartitions());
    }
    ...
    public Optional<TBucket> readUnprocessedT(String pid) {
        return Optional.ofNullable(queryStore(StateStoresConfig.UNPROCESSED_T_STORE, pid));
    }

    public Optional<SBucket> readUnprocessedS(String pid) {
        return Optional.ofNullable(queryStore(StateStoresConfig.UNPROCESSED_S_STORE, pid));
    }
