    ...
    default AllocationResult allocateForIncomingT(T incomingT, List<S> orderedCandidates, List<S> untouchedCandidates, String idPrefix) {
        ...
        for (S candidate : orderedCandidates) {
            ...
            if (allocated != 0) {
                ...
                updatedT = new T(updatedT.id(), updatedT.pid(), updatedT.pidAlt1(), updatedT.pidAlt2(), updatedT.ref(), updatedT.accId(), updatedT.sorId(), updatedT.oarId(), updatedT.tt(), updatedT.tDate(), allocatedSDate, updatedT.a_status(), updatedT.cancel(), updatedT.q(), nextTotal, allocated, updatedT.q_f(), updatedT.ledgerTime());
                ...
                updatedS.add(nextS);
                emitted.add(new TS(idPrefix + "-" + (++tsIndex), updatedT.pid(), updatedT.pidAlt1(), updatedT.pidAlt2(), updatedT.id(), nextS.id(), updatedT.accId(), updatedT.sorId(), updatedT.oarId(), updatedT.tDate(), nextS.bDate(), updatedT.q(), allocated, nextTotal, updatedT.tt(), nextS.o()));
            } else {
    ...
    default AllocationResult allocateForIncomingS(List<T> orderedCandidates, List<T> untouchedCandidates, S incomingS, String idPrefix) {
        ...
        for (T candidate : orderedCandidates) {
            ...
            if (allocated != 0) {
                ...
                T nextT = new T(candidate.id(), candidate.pid(), candidate.pidAlt1(), candidate.pidAlt2(), candidate.ref(), candidate.accId(), candidate.sorId(), candidate.oarId(), candidate.tt(), candidate.tDate(), allocatedSDate, candidate.a_status(), candidate.cancel(), candidate.q(), nextTotal, allocated, candidate.q_f(), candidate.ledgerTime());
