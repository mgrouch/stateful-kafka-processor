    ...
    default AllocationResult allocateForIncomingT(T incomingT, List<S> orderedCandidates, List<S> untouchedCandidates, String idPrefix) {
        ...
        for (S candidate : orderedCandidates) {
            ...
            if (allocated != 0) {
                ...
                updatedT = new T(updatedT.id(), updatedT.pid(), updatedT.pidAlt1(), updatedT.pidAlt2(), updatedT.ref(), updatedT.accId(), updatedT.tt(), updatedT.tDate(), allocatedSDate, updatedT.a_status(), updatedT.cancel(), updatedT.q(), nextTotal, allocated, updatedT.q_f(), updatedT.ledgerTime());
                ...
                updatedS.add(nextS);
                emitted.add(new TS(idPrefix + "-" + (++tsIndex), updatedT.pid(), updatedT.pidAlt1(), updatedT.pidAlt2(), updatedT.id(), nextS.id(), updatedT.accId(), updatedT.tDate(), nextS.bDate(), updatedT.q(), allocated, nextTotal, updatedT.tt(), nextS.o()));
            } else {
